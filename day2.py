...
for el in list:
    x = el.split() # ["range", "indicator", "string"]
    
    removeLine = x[0].split("-")
    min = int(removeLine[0])
    max = int(removeLine[1])
    ...
    charAtFirstPos = string.find(indicator, min - 1, min)
    charAtSecondPos = string.find(indicator, max -1, max)

    count = string.count(indicator)

    if( count >= min and count <= max ):
        validWords = validWords + 1
    
    if( (charAtFirstPos != -1 and charAtSecondPos == -1) or (charAtFirstPos == -1 and charAtSecondPos != -1) ):
        validWords2 = validWords2 + 1
